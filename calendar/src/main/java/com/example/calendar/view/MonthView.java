...
import com.example.calendar.listener.OnClickMonthViewListener;
import com.example.calendar.utils.Attrs;
import com.example.calendar.utils.Utils;
                ...
                if (Utils.isEqualsMonth(dateTime, mInitialDateTime)) {
                    ...
                    if (Utils.isToday(dateTime)) {
                        if (mSelectDateTime != null && dateTime.equals(mSelectDateTime)){
                            ...
                            canvas.drawCircle(rect.centerX(), centerY+3, mSelectCircleRadius, mSorlarPaint);
                            mSorlarPaint.setColor(Color.WHITE);
                            canvas.drawText(dateTime.getDayOfMonth() + "", rect.centerX(), baseline, mSorlarPaint);
                        }else if (pointList != null && pointList.contains(dateTime.toLocalDate().toString())){
                        ...
                        canvas.drawCircle(rect.centerX(),rect.bottom -  (rect.bottom-rect.top)/6, mPointSize, mLunarPaint);
                    } else if (mSelectDateTime != null && dateTime.equals(mSelectDateTime)) {
                        //绘制空心圆
//                        mSorlarPaint.setColor(mSelectCircleColor);
//                        int centerY = mRowNum == 5 ? rect.centerY() : (rect.centerY() + (mHeight / 5 - mHeight / 6) / 2);
//                        canvas.drawCircle(rect.centerX(), centerY+5, mSelectCircleRadius, mSorlarPaint);
//                        mSorlarPaint.setColor(mHollowCircleColor);
//                        canvas.drawCircle(rect.centerX(), centerY+5, mSelectCircleRadius - mHollowCircleStroke, mSorlarPaint);
